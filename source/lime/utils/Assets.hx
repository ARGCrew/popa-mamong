...
class Assets {
	...
	public static function exists(id:String, type:AssetType = null) {
		#if (tools && !display)
		if (type == null)
			type = BINARY;
		...
		if (symbol.library != null) {
			if (symbol.exists(type)) {
				if (symbol.isLocal(type)) {
					...
					CrashHandler.initCrash(type + " asset \"" + id + "\" exists, but only asynchronously", INVALIDFILE);
			}
			else
				CrashHandler.initCrash("There is no " + type + " asset with an ID of \"" + id + "\"", INVALIDFILE);
		}
		else
			CrashHandler.initCrash(__libraryNotFound(symbol.libraryName), INVALIDLIBRARY);
		#end
	 ...
	 */
	public static function getFont(id:String, useCache:Bool = true)
	 ...
	 */
	public static function getImage(id:String, useCache:Bool = true)
		return getAsset(id, IMAGE, useCache);

	public static function getLibrary(name:String) {
		if (name == null || name == "")
			name = "default";
	...
	public static function getPath(id:String) {
		...
		if (symbol.library != null) {
			if (symbol.exists())
				...
				CrashHandler.initCrash("There is no asset with an ID of \"" + id + "\"", INVALIDFILE);
		}
		else
			CrashHandler.initCrash(__libraryNotFound(symbol.libraryName), INVALIDLIBRARY);
		#end
	...
	public static function hasLibrary(name:String) {
		if (name == null || name == "")
			name = "default";
	...
	public static function isLocal(id:String, type:AssetType = null, useCache:Bool = true) {
		#if (tools && !display)
		if (useCache && cache.enabled)
	...
	private static function isValidAudio(buffer:AudioBuffer)
		return buffer != null;

	private static function isValidImage(image:Image)
		return (image != null && image.buffer != null);

	public static function list(type:AssetType = null) {
		var items = [];
	...
	public static function loadAsset(id:String, type:AssetType, useCache:Bool) {
		#if (tools && !display)
		if (useCache && cache.enabled) {
			switch (type) {
				case BINARY, TEXT:
	...
	public static function loadLibrary(id:String) {
	...
	public static function registerLibrary(name:String, library:AssetLibrary) {
	...
	public static function unloadLibrary(name:String) {
		#if (tools && !display)
		if (name == null || name == "")
			name = "default";
	...
	@:noCompletion private static function __cacheBreak(path:String) {
		#if web
		if (cache.version > 0) {
			if (path.indexOf("?") > -1)
				path += "&" + cache.version;
	...
	@:noCompletion private static function __libraryNotFound(name:String) {
		if (name == null || name == "")
			name = "default";
	...
	// Event Handlers
	@:noCompletion private static function library_onChange() {
		cache.clear();
		onChange.dispatch();
	}
}
